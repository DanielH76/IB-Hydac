...
export function loadMeetings(): Meeting[] {
  let jsonString: string = txtService.syncReadFile("meetingStore.json");
...
export function create(meetingToCreate: Meeting): boolean {
  txtService.appendJsonFileMeeting("meetingStore.json", meetingToCreate);
  loadMeetings();
  return true;
}
...
export function deleteMeeting(id: number): boolean {
  ...
  txtService.updateMeetingFileJson("meetingStore.json", meetingsTotal);
  loadMeetings();
  return true;
}
